    ...
    <div className="w-[300px] bg-white border-l border-gray-200 h-screen overflow-y-auto">
      ...
      <div className="p-4 border-b border-gray-200">
        ...
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-4">
          {/* Followers */}
          <Link to={`/profile/${authUser?.userName}/followers`} className="flex items-center gap-3 mb-3 cursor-pointer hover:opacity-80 transition-opacity">
          ...
          {/* Following */}
          <Link to={`/profile/${authUser?.userName}/following`} className="flex items-center gap-2 text-xs text-gray-500 cursor-pointer hover:text-indigo-600 transition-colors">
            ...
            upcomingBirthdays.map(user => (
              <div key={user._id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500">{formatDate(user.birthday)}</div>
                </div>
              </div>
            ))
          ) : (
            <div className="text-center text-gray-400 text-sm py-2">No upcoming birthdays</div>
          )}
        </div>
      </div>
    </div>
